...
import { membershipApi, type BillingOverview } from '../lib/membershipApi'
...
function Profile() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [nextProfile, nextOverview, nextGifts, nextContacts, nextPayments, nextBilling] = await Promise.all([
                profileApi.get(), profileApi.overview(), profileApi.gifts(), profileApi.contacts(), profileApi.payments(), membershipApi.overview(),
            ])
            setProfile(nextProfile); setOverview(nextOverview); setGifts(nextGifts); setContacts(nextContacts); setPayments(nextPayments); setBilling(nextBilling)
        } catch (loadError) {
    ...
    return <main className="mx-auto w-[calc(100%-2rem)] max-w-5xl flex-1 sm:w-[calc(100%-3rem)]">
        ...
        <Section title="Översikt">
            <div className="grid gap-4 border-b border-border pb-7 sm:grid-cols-3">
                <div><p className="text-sm text-muted-foreground">Kvarvarande poäng</p><p className="mt-1 text-3xl font-bold text-primary">{billing?.pointBalance ?? overview?.pointBalance ?? profile.pointBalance}</p></div>
            ...
            <div className="mt-6"><p className="font-semibold text-foreground">{billing?.plan?.name ?? profile.plan?.name ?? 'Inget aktivt medlemskap'}</p><p className="text-sm text-muted-foreground">{billing?.plan?.monthlyPrice ?? 0} kr/mån · {billing?.plan?.monthlyPoints ?? profile.plan?.monthlyPoints ?? 0} poäng tilldelas vid start</p>{billing?.nextAllocationDate && <p className="mt-1 text-sm text-muted-foreground">Poängperiod: {formatDate(billing.nextAllocationDate)}</p>}</div>
        </Section>
        ...
        <Section title="Kvitton"><div id="kvitton">{(billing?.payments.length ?? 0) === 0 && payments.length === 0 ? <p className="text-sm text-muted-foreground">Inga betalningskvitton finns ännu.</p> : <div className="divide-y divide-border">{billing?.payments.map((payment) => <div className="flex flex-col justify-between gap-2 py-4 sm:flex-row sm:items-center" id={`receipt-${String(payment.id)}`} key={String(payment.id)}><div className="flex items-center gap-3"><CreditCard className="size-5 text-accent" /><div><p className="font-semibold text-foreground">{payment.plan_name_snapshot}</p><p className="text-sm text-muted-foreground">{formatDate(payment.paid_at ?? payment.created_at)} · {payment.status} · Kvitto {payment.receipt_number}</p></div></div><p className="font-semibold text-foreground">{money(payment.amount, payment.currency)}</p></div>)}{billing?.payments.length === 0 && payments.map((payment) => <div className="flex justify-between gap-2 py-4" key={String(payment.id)}><span>{payment.planName} · {formatDate(payment.date)}</span><span>{money(payment.amount, payment.currency)}</span></div>)}</div>}</div>
        </Section>
