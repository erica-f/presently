...
function authenticated(req: Request, res: Response, next: NextFunction) {
    ...
    if (typeof userId !== 'string' && typeof userId !== 'number') { res.status(401).json({ error: 'Authentication required' }); return }
    res.locals.membershipUserId = userId; next()
}
...
router.get('/checkout/:plan', async (req, res) => {
    ...
    try { res.json({ plan, billing: await getBillingOverview(res.locals.membershipUserId) }) } catch { res.status(500).json({ error: 'Checkout kunde inte laddas.' }) }
})
...
router.get('/confirmation/:paymentId', async (req, res) => {
    ...
    try {
        const confirmation = await getPaymentConfirmation(res.locals.membershipUserId, req.params.paymentId)
